...
def synthesize_request_rate(cpu_util: pd.Series, base_rps: float = 100.0, variance: float = 0.3) -> pd.Series:
...
def synthesize_latency(request_rate: pd.Series, cpu_util: pd.Series) -> dict[str, pd.Series]:
    ...
    normalized_rr = (request_rate - request_rate.min()) / (request_rate.max() - request_rate.min() + 1e-6)
    normalized_cpu = cpu_util / 100.0
...
def convert_to_project_format(
    ...
    print(f"    latency_p95: mean={result['latency_p95'].mean():.3f}s, std={result['latency_p95'].std():.3f}s")
    print(f"    active_jobs: mean={result['active_jobs'].mean():.1f}, max={result['active_jobs'].max():.0f}")
